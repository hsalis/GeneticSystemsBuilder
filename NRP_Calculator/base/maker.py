...
try:
    numpy.random.bit_generator = numpy.random._bit_generator        #for Python 2
except:
    pass
...
class NRPMaker(object):
    ...
    def __init__(self, part_type='RNA', seed=None):
        ...
        if not seed is None and isinstance(seed, int):
            try:
                self.rng = numpy.random.default_rng(seed=seed)  #Python 3
            except:
                self.rng = numpy.random.RandomState(seed=seed)  #Python 2
        else:
            try:
                self.rng = numpy.random.default_rng()           #Python 3
            except:
                self.rng = numpy.random.RandomState()           #Python 2
    ...
    def nrp_maker(self,
        ...
        if check_constraints:
            ...
            if check_status == False:
                error_message = f'Invalid inputs into NRP Calculator Maker mode'
                ...
                if not background is None:
                    ...
                    if isinstance(background, kmerSetDB):
                        if background.K > len(seq_constr):
                            build_parts = False
                            error_message = '\n [ERROR] Background Lmax of {} is greater than desired part length ({}-bp)\n[SOLUTION] Try using a background with Lmax less than or equal to part length\n'.format(background.K-1, len(seq_constr))
                            if verbose: print(error_message)
                        if build_parts and not background.ALIVE:
                            build_parts = False
                            error_message = '\n [ERROR] Background is closed or dropped\n[SOLUTION] Try using an open Background\n'
                            if verbose: print(error_message)
                        if build_parts:
                            if verbose: print('\n Check Status: PASS')
                        ...
                        build_parts = False
                        error_message = '\n [ERROR] Background Object is INVALID\n[SOLUTION] Try instantiating background via nrpcalc.background(...)\n'
                        if verbose: print(error_message)
            ...
            if build_parts:
                ...
                if check_status == False:
                    error_message = 'Invalid inputs into NRP Calculator Maker mode'
                    if verbose: print(' Check Status: FAIL\n')
                    build_parts = False
                else:
                    if verbose: print('\n Check Status: PASS')
...
def main():
    ...
    try:
        ...
        background.multiadd(list(toolbox1.values()))

        # Assert All Parts Unique and Non-Repetitive
        assert len(set(toolbox1.values())) == len(
            finder.nrp_finder(list(toolbox1.values()), homology, allow_internal_repeat=False, verbose=False))
